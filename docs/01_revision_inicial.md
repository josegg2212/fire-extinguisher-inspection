...
Como `outputs/` estaba ignorado en esta fase inicial, este informe se conserva en `docs/01_revision_inicial.md`.
...
```bash
PYTHONPATH=src python3 scripts/inferir_imagen.py --help
PYTHONPATH=src python3 scripts/inferir_video.py --help
PYTHONPATH=src python3 scripts/legacy/crear_recortes_yolo.py --help
PYTHONPATH=src python3 scripts/validar_dataset.py --help
...
docker compose -f docker/docker-compose.yml run --rm --no-deps app python3 scripts/prueba_basica.py
...
- `scripts/prueba_basica.py` pasó dentro del contenedor.
...
- `scripts/inferir_imagen.py --image no_existe.jpg` devuelve JSON con error controlado.
- `scripts/validar_dataset.py` valida el ejemplo YOLO y avisa correctamente de que el dataset CNN aún no contiene imágenes.
...
- Ejecutar opcionalmente `docker compose -f docker/docker-compose.yml run --rm app python3 scripts/prueba_basica.py`.
