...
docker compose -f docker/docker-compose.yml run --rm --no-deps --user "$(id -u):$(id -g)" app \
  python3 scripts/legacy/ver_dataset_yolo.py \
...
docker compose -f docker/docker-compose.yml run --rm --no-deps --user "$(id -u):$(id -g)" app python3 scripts/prueba_basica.py
