...
> Nota 2026-04-26: este documento describe la v1 del dataset CNN en `data/classifier/`. La v1 fue util para validar el flujo de generacion, entrenamiento corto e integracion preliminar, pero queda superada para el siguiente entrenamiento porque sus crops eran demasiado ajustados al bbox YOLO y perdian contexto. La version recomendada para la proxima CNN baseline es `data/classifier_context_v2/`, documentada en `docs/08_dataset_cnn_contextual.md`.
...
```bash
PYTHONPATH=src .venv/bin/python scripts/legacy/crear_dataset_cnn_basico.py \
...
```bash
PYTHONPATH=src .venv/bin/python scripts/validar_dataset_cnn.py \
