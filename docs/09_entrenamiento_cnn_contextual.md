...
docker compose -f docker/docker-compose.yml run --rm --no-deps --user "$(id -u):$(id -g)" app-gpu \
  python3 scripts/evaluar_cnn.py \
...
La prueba anterior de v1 (`docs/07_prueba_cnn_inicial.md`) uso crops ajustados y 5 epocas. Sus metricas no son comparables directamente con v2 porque:
