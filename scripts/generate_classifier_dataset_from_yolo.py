#!/usr/bin/env python3
"""Genera un dataset CNN de estado a partir de labels YOLO reales."""
...
class ResumenGeneracion:
    """Contadores de la generacion del dataset."""
...
def crear_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Genera un dataset de clasificacion de estado desde un dataset YOLO."
    )
    parser.add_argument("--data-yaml", default="data/yolo/data.yaml", help="Ruta al data.yaml YOLO.")
    parser.add_argument("--output-dir", default="data/classifier", help="Directorio de salida.")
    parser.add_argument(
        "--max-per-split",
        type=int,
        default=None,
    ...
    parser.add_argument("--padding", type=float, default=0.10, help="Margen adicional alrededor de la bbox.")
    parser.add_argument("--seed", type=int, default=42, help="Semilla aleatoria.")
    parser.add_argument("--overwrite", action="store_true", help="Limpia el dataset CNN antes de generar.")
    parser.add_argument("--min-crop-size", type=int, default=32, help="Tamano minimo del crop aceptado.")
...
def _obtener_splits_yolo(ruta_yaml: Path, datos: dict[str, Any]) -> list[SplitYolo]:
    ...
    for nombre_yolo, nombre_salida, clave_yaml in claves:
        if clave_yaml not in datos:
            raise RuntimeError(f"Falta la clave '{clave_yaml}' en {ruta_yaml}.")
...
def _validar_args(args: argparse.Namespace) -> None:
    ...
    if args.padding < 0:
        raise RuntimeError("--padding no puede ser negativo.")
...
def _crear_estructura_salida(output_dir: Path) -> None:
    for split in SPLITS_SALIDA:
        for clase in CLASES_ESTADO:
            ruta_clase = output_dir / split / clase
            ruta_clase.mkdir(parents=True, exist_ok=True)
            (ruta_clase / ".gitkeep").write_text("\n", encoding="utf-8")


def _limpiar_salida(output_dir: Path) -> None:
    ruta_resuelta = output_dir.resolve()
    rutas_protegidas = {Path("/").resolve(), Path.home().resolve(), REPO_ROOT.resolve(), (REPO_ROOT / "data").resolve()}
...
def _bbox_yolo_a_pixeles(
    ...
    margen_x = bbox_ancho * padding
    margen_y = bbox_alto * padding
    x1 = max(0, int(round(x1 - margen_x)))
    y1 = max(0, int(round(y1 - margen_y)))
    x2 = min(ancho_imagen, int(round(x2 + margen_x)))
    y2 = min(alto_imagen, int(round(y2 + margen_y)))
    return x1, y1, x2, y2
...
def _procesar_objeto(
    ...
    x1, y1, x2, y2 = _bbox_yolo_a_pixeles(anotacion, ancho_imagen, alto_imagen, args.padding)
    if x2 - x1 < args.min_crop_size or y2 - y1 < args.min_crop_size:
        resumen.crops_descartados += 1
        return

    crop_visible = _resize_crop(imagen.crop((x1, y1, x2, y2)), args.image_size)
    ...
    for indice in range(args.partial_occlusions_per_object):
        crop_partial = aplicar_oclusion_parcial(crop_visible, rng)
    ...
    for indice in range(args.blocked_occlusions_per_object):
        crop_blocked = aplicar_oclusion_fuerte(crop_visible, rng)
...
def _imprimir_resumen(resumen: ResumenGeneracion, output_dir: Path) -> None:
    print(f"Dataset CNN generado en: {output_dir}")
    for split in SPLITS_SALIDA:
