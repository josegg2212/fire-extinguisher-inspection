...
def crear_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help="Ruta opcional a una segunda CNN que solo corrige partially_occluded a visible.",
    )
    ...
    parser.add_argument("--yolo-imgsz", type=int, default=None, help="Tamaño de imagen usado por YOLO.")
...
def main() -> int:
    ...
    if args.visibility_verifier_model is not None:
        cambios_modelos["visibility_verifier"] = Path(args.visibility_verifier_model)
    ...
    if args.yolo_imgsz is not None:
        if args.yolo_imgsz <= 0:
            print("ERROR: --yolo-imgsz debe ser mayor que 0.", file=sys.stderr)
            return 2
        cambios_inferencia["yolo_imgsz"] = args.yolo_imgsz
    ...
    if args.visibility_verifier_min_visible_probability is not None:
        if not 0 <= args.visibility_verifier_min_visible_probability <= 1:
            print("ERROR: --visibility-verifier-min-visible-probability debe estar entre 0 y 1.", file=sys.stderr)
            return 2
        cambios_inferencia["visibility_verifier_min_visible_probability"] = (
            args.visibility_verifier_min_visible_probability
        )
    if args.visibility_verifier_primary_min_visible_probability is not None:
        if not 0 <= args.visibility_verifier_primary_min_visible_probability <= 1:
            print(
                "ERROR: --visibility-verifier-primary-min-visible-probability debe estar entre 0 y 1.",
                file=sys.stderr,
            )
            return 2
        cambios_inferencia["visibility_verifier_primary_min_visible_probability"] = (
            args.visibility_verifier_primary_min_visible_probability
        )
    if args.visibility_verifier_primary_max_blocked_probability is not None:
        if not 0 <= args.visibility_verifier_primary_max_blocked_probability <= 1:
            print(
                "ERROR: --visibility-verifier-primary-max-blocked-probability debe estar entre 0 y 1.",
                file=sys.stderr,
            )
            return 2
        cambios_inferencia["visibility_verifier_primary_max_blocked_probability"] = (
            args.visibility_verifier_primary_max_blocked_probability
        )
    if cambios_modelos:
