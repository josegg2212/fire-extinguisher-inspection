#!/usr/bin/env python3
"""Evalúa el pipeline completo de inspección sobre un conjunto de imágenes."""
...
def crear_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evalúa el pipeline completo YOLO + CNN.")
    parser.add_argument("--images-dir", default="data/yolo/test/images", help="Directorio con imágenes a procesar.")
    parser.add_argument(
        "--yolo-model",
        default="models/yolo/extinguisher_yolo_v1/weights/best.pt",
        help="Peso YOLO entrenado.",
    )
    parser.add_argument(
        "--classifier-model",
        default="models/classifier/state_classifier_context_v7_real_tuned_with_manual_tests.pt",
        help="Checkpoint CNN contextual ajustado con validación manual.",
    )
    parser.add_argument(
        "--visibility-verifier-model",
        default=None,
        help="Checkpoint opcional de una segunda CNN que solo corrige partially_occluded a visible.",
    )
    parser.add_argument(
        "--output-dir",
        default="outputs/detections/evaluacion_pipeline",
        help="Directorio de salidas del pipeline.",
    )
    parser.add_argument("--max-images", type=int, default=50, help="Máximo de imágenes a procesar.")
    parser.add_argument(
    ...
    parser.add_argument("--confidence-threshold", type=float, default=0.05, help="Umbral de confianza YOLO.")
    parser.add_argument("--yolo-imgsz", type=int, default=1920, help="Tamaño de imagen usado por YOLO.")
    parser.add_argument(
        "--visibility-verifier-min-visible-probability",
        type=float,
        default=None,
        help="Probabilidad visible mínima del verificador para promocionar a visible.",
    )
    parser.add_argument(
        "--visibility-verifier-primary-min-visible-probability",
        type=float,
        default=None,
        help="Probabilidad visible mínima del clasificador principal para consultar el verificador.",
    )
    parser.add_argument(
        "--visibility-verifier-primary-max-blocked-probability",
        type=float,
        default=None,
        help="Probabilidad blocked máxima del clasificador principal para consultar el verificador.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="outputs/reports/evaluacion_pipeline_contact_sheet.jpg",
...
def _validar_entradas(args: argparse.Namespace) -> tuple[Path, Path, Path, Path | None, Path]:
    images_dir = _resolver(args.images_dir)
    yolo_model = _resolver(args.yolo_model)
    classifier_model = _resolver(args.classifier_model)
    visibility_verifier_model = (
        _resolver(args.visibility_verifier_model) if args.visibility_verifier_model is not None else None
    )
    ...
    if visibility_verifier_model is not None and not visibility_verifier_model.exists():
        faltantes.append(f"verificador de visibles: {visibility_verifier_model}")
    if faltantes:
        detalle = "\n- ".join(faltantes)
    ...
    if args.yolo_imgsz <= 0:
        raise RuntimeError("--yolo-imgsz debe ser mayor que 0.")
    ...
    if (
        args.visibility_verifier_min_visible_probability is not None
        and not 0 <= args.visibility_verifier_min_visible_probability <= 1
    ):
        raise RuntimeError("--visibility-verifier-min-visible-probability debe estar entre 0 y 1.")
    if (
        args.visibility_verifier_primary_min_visible_probability is not None
        and not 0 <= args.visibility_verifier_primary_min_visible_probability <= 1
    ):
        raise RuntimeError("--visibility-verifier-primary-min-visible-probability debe estar entre 0 y 1.")
    if (
        args.visibility_verifier_primary_max_blocked_probability is not None
        and not 0 <= args.visibility_verifier_primary_max_blocked_probability <= 1
    ):
        raise RuntimeError("--visibility-verifier-primary-max-blocked-probability debe estar entre 0 y 1.")

    return images_dir, yolo_model, classifier_model, visibility_verifier_model, output_dir
...
def _crear_config_pipeline(
    ...
    visibility_verifier_model: Path | None,
    output_dir: Path,
    ...
    visibility_verifier_min_visible_probability: float | None,
    visibility_verifier_primary_min_visible_probability: float | None,
    visibility_verifier_primary_max_blocked_probability: float | None,
) -> Any:
    ...
    modelos = replace(
        ...
        visibility_verifier=visibility_verifier_model,
    )
    outputs = replace(
        config.outputs,
    ...
    if visibility_verifier_min_visible_probability is not None:
        inferencia = replace(
            inferencia,
            visibility_verifier_min_visible_probability=visibility_verifier_min_visible_probability,
        )
    if visibility_verifier_primary_min_visible_probability is not None:
        inferencia = replace(
            inferencia,
            visibility_verifier_primary_min_visible_probability=visibility_verifier_primary_min_visible_probability,
        )
    if visibility_verifier_primary_max_blocked_probability is not None:
        inferencia = replace(
            inferencia,
            visibility_verifier_primary_max_blocked_probability=visibility_verifier_primary_max_blocked_probability,
        )
...
def _crear_resumen(
    ...
    return {
        "tipo": "evaluacion_pipeline",
        "modelos": {
            "yolo": args.yolo_model,
            "classifier": args.classifier_model,
            "visibility_verifier": args.visibility_verifier_model,
        },
        ...
        "visibility_verifier_min_visible_probability": args.visibility_verifier_min_visible_probability,
        "visibility_verifier_primary_min_visible_probability": (
            args.visibility_verifier_primary_min_visible_probability
        ),
        "visibility_verifier_primary_max_blocked_probability": args.visibility_verifier_primary_max_blocked_probability,
...
def main() -> int:
    ...
    try:
        images_dir, yolo_model, classifier_model, visibility_verifier_model, output_dir = _validar_entradas(args)
        ...
        config = _crear_config_pipeline(
            ...
            visibility_verifier_model=visibility_verifier_model,
            ...
            classifier_square_crop=args.classifier_square_crop,
            visibility_verifier_min_visible_probability=args.visibility_verifier_min_visible_probability,
            visibility_verifier_primary_min_visible_probability=args.visibility_verifier_primary_min_visible_probability,
            visibility_verifier_primary_max_blocked_probability=args.visibility_verifier_primary_max_blocked_probability,
        )
        ...
        resumen_path = output_dir / "resumen_pipeline.json"
    ...
    print(f"Imágenes con detección: {resumen['imagenes_con_deteccion']}")
    print(f"Imágenes sin detección: {resumen['imagenes_sin_deteccion']}")
    ...
    if resumen["clases_esperadas"]:
        ...
        for clase, total in sorted(resumen["clases_esperadas"].items()):
            print(f"- {clase}: {total}")
        print(
            "Accuracy de estado sobre imágenes con detección: "
